...
def back_prop(tmps, minibatch_class):
    ...
    derivs["da" + str(layer_num-1)], derivs["dw" + str(layer_num)], derivs["db" +
                                                                           str(layer_num)] = back_prop_step(None, tmps[-1], "softmax", 1.5, minibatch_class)
    for i in reversed(range(layer_num-1)):
        da_prev, dw, db = back_prop_step(
            derivs["da" + str(i+1)], tmps[i], "relu", 1.5, minibatch_class)
...
def learn(train_set, train_class, learning_rate, batch_num, epochs):
    ...
    for i in range(0, epochs):
        ...
        for minibatch in train_sets:
            ...
            cost_prev = cost
            cost = ce_loss_l2(a, train_classes[j], params, 1.5)
...
path = './costs/kadai_15_0.txt'
f = open(path,'w')
f.write(str(costs[-1]))
f.close

with open('./params_15_0.pickle', mode='wb') as f:
...
fig.savefig("./result/cost_15_0.png")
# plt.show()
...
plt.ylabel('accuracy')
plt.legend(loc="lower right")
fig.savefig("./result/accuracy_15_0.png")
# plt.show()
